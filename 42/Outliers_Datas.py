...
#fig.savefig("C:/Users/Romain/Desktop/Mesures/Images/Outliers_Datas.pdf", bbox_inches='tight', transparent=True, pad_inches=0.5)
...
def log_likelihood(theta, x, y, e, sigma_B):
    ...
    logL2 = np.log(1 - g) - 0.5 * np.log(2 * np.pi * sigma_B ** 2) - 0.5 * (dyA / sigma_B) ** 2
...
nsteps = nburn + 1000  # number of MCMC steps to take

# set theta near the maximum likelihood, with 
np.random.seed(0)
starting_guesses = np.zeros((nwalkers, ndim))
starting_guesses[:, 0] = np.random.normal(-10, 10, (nwalkers))
starting_guesses[:, 1] = np.random.normal(1.5, 2.5, (nwalkers))
...
# Fonctions de tracés graphiques
def compute_sigma_level(trace1, trace2, nbins=20):
    """Echantillonnage par nombre d'écarts types"""
...
def plot_MCMC_trace(ax, xtrace,ytrace, scatter=False, **kwargs):
    """Plot traces and contours"""
    xbins, ybins, sigma = compute_sigma_level(xtrace, ytrace)
    ax.contour(xbins, ybins, sigma.T, levels=[0.683, 0.955], **kwargs)
    ax.contour(xbins, ybins, sigma.T, levels=[0.25], colors = 'red', zorder =2)
    if scatter:
        ax.plot(xtrace, ytrace, ',k', alpha=0.05)
...
plot_MCMC_trace(ax,b_sample, a_sample, True, colors='k')

#plt.plot(b_sample, a_sample, ',k', alpha=0.1)
plt.xlabel("Ordonnée à l'origine b")
plt.ylabel('Pente a');
plt.text(-40,2,'estimation de a : {:.2f}'.format(np.mean(a_sample)))
plt.text(-40,1.8,'estimation de b : {:.2f}'.format(np.mean(b_sample)))
fig.savefig("C:/Users/Romain/Desktop/Mesures/Images/SlopeIntercept.pdf", bbox_inches='tight', transparent=True, pad_inches=0.5)
